...
async function handlePopupRecovery() {
  ...
  try {
    ...
    if (result.success) {
      const successMsg = document.getElementById('recoveryMessage');
      if (successMsg) {
        successMsg.textContent = 'Instruções enviadas para seu e-mail!';
    ...
    const successMsg = document.getElementById('recoveryMessage');
    if (successMsg) {
      successMsg.textContent = 'Instruções enviadas (modo offline)!';
      successMsg.classList.add('show');
    }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Botão fechar recuperação
  const closeRecoveryBtn = document.getElementById('closeRecoveryBtn');
  if (closeRecoveryBtn) {
    closeRecoveryBtn.addEventListener('click', (e) => {
      e.preventDefault();
      closeRecoveryPopup();
      showLoginPopup();
    });
...
function logout() {
  ...
  const emailInput = document.getElementById('email');
  const passwordInput = document.getElementById('password');
  const rememberCheckbox = document.getElementById('rememberPopup');
